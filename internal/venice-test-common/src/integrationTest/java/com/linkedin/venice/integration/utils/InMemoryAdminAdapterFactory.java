 ...
 * {@link PubSubAdminAdapterContext#getPubSubBrokerAddress()} at create() time. Returns
 * a {@link MockInMemoryAdminAdapter} wired to that broker, so controller-issued topic
 * CRUD operations modify the same broker that the producer/consumer adapters of this
 * region use.
 */
public class InMemoryAdminAdapterFactory extends PubSubAdminAdapterFactory<PubSubAdminAdapter> {
  ...
  public PubSubAdminAdapter create(PubSubAdminAdapterContext adminAdapterContext) {
    ...
    // Reuse the same MockInMemoryAdminAdapter instance that InMemoryConsumerAdapterFactory
    // wires into MockInMemoryConsumerAdapter.partitionsFor(). The mock's topic CRUD state
    // is held inside the admin adapter (not the broker), so all admin/consumer pairs for
    // the same broker must share the SAME admin instance.
