...
/**
 * Phase 9: parallel of {@link MockInMemoryAdminAdapter} that targets
 * {@link BoundedInMemoryPubSubBroker}. Behaviour is identical; only the broker type
 * differs. (The existing admin adapter is typed against
 * {@link com.linkedin.venice.pubsub.mock.InMemoryPubSubBroker} and Phase 9 forbids
 * inheriting from that broker, so we composition-wire a parallel admin adapter.)
 *
 * <p>Topic CRUD state is held inside this class; the broker only knows about the topics
 * that were created via this admin adapter (which calls broker.createTopic). All
 * admin/consumer pairs for the same broker must share the SAME admin instance, which
 * the registry guarantees.
 */
public class BoundedMockInMemoryAdminAdapter implements PubSubAdminAdapter {
  ...
  private final BoundedInMemoryPubSubBroker boundedBroker;

  public BoundedMockInMemoryAdminAdapter(BoundedInMemoryPubSubBroker boundedBroker) {
    this.boundedBroker = boundedBroker;
  }
  ...
  public void createTopic(
    ...
    if (!topicPubSubTopicConfigurationMap.containsKey(topic)) {
      boundedBroker.createTopic(topic.getName(), numPartitions);
    }
